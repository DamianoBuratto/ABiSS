...
echo -e "\t > Program_Initialization.sh and custom_mdpFILE"
# GLOBAL VARIABLE
...
function Program_Initialization {
  ...
  $PYTHON "${FUNCTIONs_FOLDER}/Custom_mdpFILE.py" "${MDPs_FOLDER}/SAMD_custom.mdp" -KEY_define "${KEY_define_SAMD}" \
    ...
    -KEY_SA_temp "${KEY_SA_temp} ${KEY_SA_temp}" -KEY_compressed "${KEY_compressed}" || exit
  $PYTHON "${FUNCTIONs_FOLDER}/Custom_mdpFILE.py" "${MDPs_FOLDER}/NVT_custom.mdp" -KEY_define "-DPOSRES" -KEY_nsteps "${KEY_nsteps_NVT}" \
    -KEY_nstouts "${KEY_nstouts_NVT}" -KEY_compressed "${KEY_compressed}" || exit
  $PYTHON "${FUNCTIONs_FOLDER}/Custom_mdpFILE.py" "${MDPs_FOLDER}/NPT_custom.mdp" -KEY_define "-DPOSRES" -KEY_nsteps "${KEY_nsteps_NPT}" \
    -KEY_nstouts "${KEY_nstouts_NPT}" -KEY_compressed "${KEY_compressed}" || exit
  $PYTHON "${FUNCTIONs_FOLDER}/Custom_mdpFILE.py" "${MDPs_FOLDER}/EngComp_ff14sb_custom.mdp" -KEY_define "${KEY_define_MD}" -KEY_nsteps "${KEY_nsteps_MD}" \
    -KEY_nstouts "${KEY_nstouts_MD}" -KEY_pcoupl "${KEY_pcoupl}" -KEY_compressed "${KEY_compressed}" || exit
  # same as EngComp_ff14sb_custom.mdp but without POSRES and no compressed trj (the keyword change with gromacs version)
  $PYTHON "${FUNCTIONs_FOLDER}/Custom_mdpFILE.py" "${MDPs_FOLDER}/Protein_EngComp_ff14sb_custom.mdp" -KEY_nsteps "${KEY_nsteps_MD}" \
    -KEY_nstouts "${KEY_nstouts_MD}" -KEY_pcoupl "${KEY_pcoupl}" || exit
