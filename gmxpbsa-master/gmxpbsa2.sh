...
# clear
check ${INPUT_FILE} ERROR_NO_INPUT
...
if [ -d $RUN ]; then
	...
	cd $RUN
	cp ../${INPUT_FILE} ./run${run}_parameters2.in
	...
	echo  -e "\n"$(date +%H:%M:%S)"\n The directory $RUN is not present. Please check the ${INPUT_FILE} file or run \"gmxpbsa0.sh\"."
	echo "Exiting --"
	exit
...
for Folder in $FolderFile; do
    ...
    if [ ! -f DONE1_$Folder ]; then
    	...
    	RecoverJobs=$(awk -v Var="RecoverJobs" '{ if($1==Var) print $2}' ../${INPUT_FILE});
	...
	if [ -z "$RecoverJobs" ]; then
		echo -e "In the directory $Folder there are $failed_apbs APBS calculations (corresponding to the $perc_apbs% of the calculations) and $failed_Hapbs HAPBS calculations (corresponding to the $perc_Hapbs% of the calculations) that didn't finsh in a correct way. Please check the file $RUN/$Folder/APBS_CALCULATIONS/Failed_frame.dat. If you want to try to recover these frames set the variable \"RecoverJobs\" in the ${INPUT_FILE} file to \"y\", instead set it to \"n\". \n "  | tee -a REPORTFILE2
			var_exit=1;
	else
		if [ "$RecoverJobs" == "n" ]; then
				echo -e "WARNING!!! In the directory  $Folder the $failed failed frames won't be recovered since the variable \"RecoverJobs\" has been set to \"n\" in the ${INPUT_FILE} file, however please check the file $RUN/$Folder/APBS_CALCULATIONS/Failed_frame.dat and consider to recover these frames later."  | tee -a REPORTFILE2
		else
#----------------------------Recover Jobs-----------------------------------------------------------
...
if [ $cas == "n" ]; then
	...
	for Folder in $FolderFile; do
		...
		#		done
		#		res_type=`egrep "^MUTATION[[:blank:]]*${start_dir[$i_ref]}[[:blank:]]" ../${INPUT_FILE}| head -n ${val_start_dir[$i_ref]} |tail -n 1 | awk '{if($1=="MUTATION") print $4}'`
		#		res_id=`egrep "^MUTATION[[:blank:]]*${start_dir[$i_ref]}[[:blank:]]" ../${INPUT_FILE}| head -n ${val_start_dir[$i_ref]}|tail -n 1 | awk '{if($1=="MUTATION") print $3}'`
		#		prot_lig=`egrep "^MUTATION[[:blank:]]*${start_dir[$i_ref]}[[:blank:]]" ../${INPUT_FILE}| head -n ${val_start_dir[$i_ref]}|tail -n 1 | awk '{if($1=="MUTATION") print $5}'`
